...
public class Agent : MonoBehaviour
{
    ...
    void Update()
    {
        if(curState != State.STOP)
        {
            ...
            if (!audioSource.isPlaying && vel != Vector3.zero && grounded)
            {
                if (curStep == 0)
                    ++curStep;
                else
    ...
    void Jump()
    {
        ...
        obstacleBool = obstacleBool || (obstacleTimer.IsComplete(false) && !grounded);
    ...
    void Pusher()
    {
        ...
        if (grounded)
        {
            ...
            if (obstacleBool)
            {
                ...
                if ((!(target is ObjectNode) && Vector3.Distance(target.transform.position, transform.position) < stoppingDistance) || 
                    target.transform.position.z < transform.position.z + stoppingDistance)
                {
                    vel = Vector3.zero;
                    obstacleBool = false;
    ...
    void Wrecker()
    {
        dist = Vector3.Distance(transform.position, target.transform.position);
        // only move when obstaclebool is true
        obstacleBool = !(target is ObjectNode node) || (Mathf.Abs(node.obj.rotation.z) < 5f * Mathf.Deg2Rad) | obstacleBool;
    ...
    bool IsGrounded()
    {
        if (Physics.Raycast(transform.position, Vector3.down, out RaycastHit hit, agent.height / 1.9f))
        {
